...
```bash
ENV_FILE=$( [ -f .env ] && echo .env || echo "$HOME/.config/smart-house/env" )
SMART_HOUSE_URL=$(grep '^SMART_HOUSE_URL=' "$ENV_FILE" 2>/dev/null | cut -d= -f2-)
PORT=$(grep '^PORT=' "$ENV_FILE" 2>/dev/null | cut -d= -f2)
export BASE=${SMART_HOUSE_URL:-http://127.0.0.1:${PORT:-8080}}
```

(читать из файла только эти две строки, файл целиком не открывать — там
секреты). Skill подключён и глобально: если в текущем каталоге нет `.env`
(запуск не из репозитория), адрес берётся из `~/.config/smart-house/env` —
там только строка `SMART_HOUSE_URL=http://<домашний сервер>:<порт>`.
`SMART_HOUSE_URL` задан — сервер на другой машине (домашний сервер в
локальной сети); пусто — сервер на этом компьютере, порт из `PORT`, по
умолчанию 8080 (не localhost — на некоторых машинах :8080 занят чем-то
ещё, например Docker Desktop, и localhost уходит не туда).
